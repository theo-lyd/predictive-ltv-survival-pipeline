...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Ingest churn, promotions, and billing into Bronze append-only tables")
...
def main() -> None:
    ...
    if "monthly_recurring_revenue" in churn_pdf.columns:
        churn_pdf["monthly_recurring_revenue"] = churn_pdf["monthly_recurring_revenue"].apply(normalize_numeric_abbrev)
    ...
    churn_df = spark.createDataFrame(churn_pdf, schema=churn_schema).withColumn("ingestion_run_id", F.lit(run_id)).withColumn(
    ...
    if args.airbyte_enabled and billing_raw.exists():
        ...
        if "invoice_amount" in billing_pdf.columns:
            billing_pdf["invoice_amount"] = billing_pdf["invoice_amount"].apply(normalize_numeric_abbrev)
    ...
    reconciliation = pd.DataFrame(
        [
            {"run_id": run_id, "table_name": "churn", "rows_ingested": record["tables"]["churn"]["rows_ingested"]},
            {
                "run_id": run_id,
                "table_name": "promotions",
                "rows_ingested": record["tables"]["promotions"]["rows_ingested"],
            },
            {"run_id": run_id, "table_name": "billing", "rows_ingested": record["tables"]["billing"]["rows_ingested"]},
        ]
    )
    reconciliation.to_csv(audit_path / "row_count_reconciliation.csv", mode="a", header=not (audit_path / "row_count_reconciliation.csv").exists(), index=False)
